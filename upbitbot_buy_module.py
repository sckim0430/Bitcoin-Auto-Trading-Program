                ...
                for index,s in enumerate(stack_markets_order):
                    ...
                    if day_markets[stack_markets_order_index[index]] and (s>=BUY_STACK) and (markets[stack_markets_order_index[index]] not in buy_markets) and (markets[stack_markets_order_index[index]] not in ordered_markets):               
                        ...
                        if positive_price>=MIN_ORDER_PRICE * (1/FEE):
                            vol_check = pu.get_ohlcv(markets[stack_markets_order_index[index]],interval='minute1',count=200)
                            time.sleep(SLEEP_TIME)
                            ...
                            twenty_time_line = np.asarray(list(time_info.rolling(20).mean()[-6:]))
                            ten_time_line =  np.asarray(list(time_info.rolling(10).mean()[-6:]))
                            five_time_line = np.asarray(list(time_info.rolling(5).mean()[-6:]))
                            
                            is_accend = (five_time_line[-1]>=ten_time_line[-1]) and (ten_time_line[-1]>=twenty_time_line[-1])
                            is_near_point = (len(set(five_time_line>=ten_time_line))==2) and (len(set(ten_time_line>=twenty_time_line))==2)

                            if is_accend and is_near_point and (vol_now>=vol_avg*BUY_STACK): 
                                BUY_BALANCE = positive_price/(MAX_COIN_COUNT-len(buy_markets))
                                order_price = BUY_BALANCE*FEE
                
                                buy_market_order(markets[stack_markets_order_index[index]],order_price)
                                time.sleep(SLEEP_TIME)
...
if __name__ == "__main__":
    ...
    main(FEE,MIN_ORDER_PRICE,BUY_STACK,SLEEP_TIME,CHECK_TIME,DAY_CHECK_TIME,MAX_COIN_COUNT,markets,market_count,day_markets)
