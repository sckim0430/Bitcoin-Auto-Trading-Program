...
def get_ohlcv(ticker="KRW-BTC", interval="day", count=200, to=None):
    ...
    try:
        ...
        df = pd.DataFrame(contents, columns=['opening_price', 'high_price', 'low_price', 'trade_price','candle_acc_trade_price',
                                             'candle_acc_trade_volume'],
                          index=dt_list)
        df = df.rename(
            columns={"opening_price": "open", "high_price": "high", "low_price": "low", "trade_price": "close","cnadle_acc_trade_price":"price",
